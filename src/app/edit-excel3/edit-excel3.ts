...
@Component({
  selector: 'app-edit-excel3',
  standalone: true,
  imports: [HotTableModule,  FormsModule, ], // JsonPipe ],//
  templateUrl: './edit-excel3.html',
  styleUrl: './edit-excel3.scss'
})
export class EditExcel3 {
  ...
  async loadTemplateHTTP(id: number) {
    //temp test
    let buffer2 = await firstValueFrom(this.http.get(`http://localhost:3000/table-e/exec/${id}?ts=${this.ts}&from=${this.ts}&to=${this.ts_to}`));

    //let buffer = await firstValueFrom(this.http.get('http://localhost:3000/download', { responseType: 'arraybuffer' }));
    let buffer = await firstValueFrom(this.http.get(`http://localhost:3000/table-e/download/${id}`, { responseType: 'arraybuffer' }));
